...
def read_data(tokenizer, filetrain='train_pharagraph_full.jl', filetarget='target_strings_full.jl'):
    ...
    inputs = tokenizer.batch_encode_plus(training_input[:100], return_tensors="pt", padding=True, max_length = encoder_maxlen, truncation = True, add_special_tokens= True)
    targets = tokenizer.batch_encode_plus(targets_input[:100], return_tensors="pt", padding=True, max_length = decoder_maxlen, truncation = True, add_special_tokens= True)
    val_inputs = tokenizer.batch_encode_plus(training_input[100:110], return_tensors="pt", padding=True, max_length = encoder_maxlen, truncation = True, add_special_tokens= True)
    val_targets = tokenizer.batch_encode_plus(targets_input[100:110], return_tensors="pt", padding=True, max_length = decoder_maxlen, truncation = True, add_special_tokens= True)
